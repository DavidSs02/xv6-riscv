...
vma_free_pages(struct vma *vma, int index, uint64 init_va, uint64 end_va, pagetable_t pagetable){
  ...
  for (uint64 page = init_va; page < end_va; page += PGSIZE){
    ...
    if ((*pte & PTE_D) != 0 && vma->flags[index] == MAP_SHARED){
...
int
vma_fill(struct vma *vma, int vma_index, uint64 addr, int length, int prot, int flags, int fd, int offset, struct file *file){
