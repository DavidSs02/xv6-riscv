...
uint64
mmap(uint64 addr, int length, int prot, int flags,int fd, struct file *file, int offset, int valid_addr) {
  struct proc *p = myproc();
...
int correct_filled = vma_fill(&(p->vma_list),free_vma_index,addr,length,prot,flags,fd,offset,file);
if (correct_filled == -1){
  return -1;
}
filedup(file);
...
munmap(uint64 addr, int length){
  ...
  if (p->vma_list.addr[vma_index] == page_addr){
    ...
    vma_fill(&(p->vma_list), new_vma_index, addr_p2, length_p2, p->vma_list.prot[vma_index], 
